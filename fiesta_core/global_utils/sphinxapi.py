                ...
                for i in range(len(attrs)):
                    if attrs[i][1] == SPH_ATTR_FLOAT:
                        ...
                        for n in range(0, nvals, 1):
                            ...
                            p += 8
                        p -= 4
                    elif attrs[i][1] == SPH_ATTR_TIMESTAMP:
                        timestamp = unpack('>L', response[p:p + 4])[0]
                        if timestamp and timestamp > 0:
                            match['attrs'][attrs[i][0]] = datetime.datetime.fromtimestamp(timestamp)
                        else:
                            match['attrs'][attrs[i][0]] = None
