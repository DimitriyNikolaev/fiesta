...
class UnicUserMiddleware(object):
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        if settings.UNIC_TMP_USER_CITY not in request.COOKIES:
            request.COOKIES[settings.UNIC_TMP_USER_CITY] = 0
        return None
    ...
    def process_response(self, request, response):
        ...
        user_city = request.COOKIES[settings.UNIC_TMP_USER_CITY] if settings.UNIC_TMP_USER_CITY in request.COOKIES else 0
